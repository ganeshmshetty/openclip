...
public struct AIConfigureForm: View {
    ...
    /// Expanded state of the inline "how to authenticate" note. It was a popover opened from inside
    /// a popover, for two sentences and a button.
    @State private var showingAuthHelp: Bool = false

    /// `true` when the caller supplies the surrounding `Form` — the AI page appends a row that
    /// drills into the AI action library.
            ...
            Section(header: Text("Provider Settings")) {
                if aiManager.activeProviderType == .apple {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Button {
                            withAnimation(.easeInOut(duration: 0.18)) { showingAuthHelp.toggle() }
                        } label: {
    ...
    /// Inline replacement for the old authentication popover: same copy, expanding the row it
    /// explains instead of opening a layer over it.
