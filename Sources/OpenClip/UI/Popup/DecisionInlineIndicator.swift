...
// tick or a red cross for yes/no, the chosen label for a choice or score, or a warning when the
// provider failed. Drawn by the sub-bar (`.bar`, label text replaces the icon) and the palette
...
public enum DecisionInlineState: Equatable, Sendable {
    ...
    /// The state for a finished decision: the first answer decides the glyph.
    public init(_ presentation: DecisionPresentation) {
        guard let first = presentation.answers.first else {
            self = .failed
            return
        }
        switch first.value {
