...
public struct PopupView: View {
    ...
    /// Runs a free-form instruction from the Ask AI composer on the selection once. nil falls
    /// back to the view's own streaming flow (preview/static hosts).
    public let onRunAIPrompt: (@MainActor (String) -> Void)?
    /// Saves a composer instruction as a reusable AI tool and runs it. nil falls back to the
    /// view's own flow.
    ...
    /// Search mode's surface: the Ask AI composer when the scope's parent composes prompts (the
    /// Ask AI bar entry), otherwise the action-search palette.
    @ViewBuilder
    ...
    /// The Ask AI composer, hosted like the palette (same scope plumbing, key mode and Esc).
    private var promptComposer: some View {
        PromptComposerView(
            context: context,
            recents: promptHistory.prompts,
            maxSize: modeStore.searchPaletteSize,
            onRun: { instruction in
                if let onRunAIPrompt {
                    // Same contract as onRunAI: the controller's flow snapshots the selection and
                    // dismisses the popup itself, so the composer must not exit first.
            ...
            onSave: { instruction in
                if let onSaveAIPrompt {
                    ...
                    let preset = aiManager.preset(matchingPrompt: instruction)
                        ?? aiManager.addCustomPreset(title: AIPromptText.toolTitle(for: instruction), prompt: instruction)
    ...
    private func actionButton(action: any Action, index: Int, isHovered: Bool, isActiveParent: Bool = false) -> some View {
        ...
        Group {
            ...
            case .perform:
                ...
                } else if action.chrome.composesPrompt {
                    // Ask AI swaps the bar for the prompt composer: a scoped palette with no
                    // children whose parent composes prompts (see searchCard). No sub-bar, so
                    // no dwell.
                    Button {
                        onCancelSubBarDwell?()
                        let frame = hoverFrames[.action(index)]
