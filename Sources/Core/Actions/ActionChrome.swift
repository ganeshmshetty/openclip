...
public struct ActionChrome: Codable, Sendable, Equatable {
    ...
    /// True when the action opens the popup's AI prompt composer (the "Ask AI" bar entry): the
    /// bar swaps itself for a field that takes a free-form instruction for the selection. Never a
    /// palette entry, never groupable or bindable — it is a surface, not a leaf action.
