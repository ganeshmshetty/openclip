...
- Any new subprocess-spawning action must join the existing `ShellProcessRunner` executor rather than spawning its own. The shared watchdog starts cleanup at `Constants.scriptTimeout` (60 s): it terminates the child, the process group when the child is the leader, and remaining descendants. `waitUntilExit()` waits only for the direct child; descendant SIGKILL may finish afterward. Read pipes via GCD `readabilityHandler` — never a blocking `readToEnd()`.
