...
- Any new subprocess-spawning action must join the existing `ShellProcessRunner` executor rather than spawning its own; `shell`, `scriptfile`, `applescript` (via `osascript`), and `shortcut` all route through it. `OpenClipJSHost` is the exception — it bounds its own async phase with `TimeoutFlag` instead of a subprocess. The shared watchdog starts cleanup at `Constants.scriptTimeout` (60 s): it terminates the child, the process group when the child is the leader, and remaining descendants. `waitUntilExit()` waits only for the direct child; descendant SIGKILL may finish afterward. Read pipes via GCD `readabilityHandler` — never a blocking `readToEnd()`.
