...
- **Liquid Glass must stay availability-gated.** `glassSurface`/`.glassEffect` require macOS 26; the project targets macOS 14.0. Any glass surface (popup bar, bubble cards, Preferences sidebar) must keep the `#available(macOS 26, *)` branch with an `.ultraThinMaterial` fallback so Glass still renders as a frosted material on macOS 14-15. Don't stack a second dimming layer under a glass card — the glass surface itself is the single layer.
- **Standard windows unless a chrome-less card is the actual goal.** The Preferences window is the Apple-aligned pattern: standard window chrome (`.titled`, `.fullSizeContentView`, `.hiddenTitleBar`/transparent titlebar) with the glass/material surface on the *content*; the window server supplies the shadow, rounded corners, and resizing. A `.borderless` transparent NSWindow forfeits all of that, so every piece — rounded corners, border, shadow, inset room — must be re-drawn manually. Prefer stock chrome unless a floating card with no window frame is the deliberate design.
- **Onboarding is a solid card, not a glass surface.** `OnboardingWindowController` is a `.borderless` transparent window (`isOpaque = false`, `backgroundColor = .clear`, `hasShadow = false`); `OnboardingView` draws a solid `windowBackgroundColor` rounded rect + `strokeBorder` + SwiftUI shadow, hosted in a transparent container inset ~32pt so the shadow renders inside the window. A first-launch wizard is content, not navigation — keep it off Liquid Glass. Don't reintroduce `glassSurface` on onboarding.
- **Popup and bubble shadows render inside the panel.** The popup panel and onboarding window are shadowless (`hasShadow = false`); SwiftUI draws the shadows. Keep ≥16pt of SwiftUI padding around popup bar and bubble content (12pt for info bubbles) so shadows aren't clipped at the panel edge. If a shadow looks cut off, increase the padding — never re-enable the window/panel shadow (double artifacts).
- **`PopupPreview` is a static visual, never a live registry snapshot.** It hardcodes the canonical action set (Search, Copy, Cut, Paste, Services) plus the AI sparkles (`PopupView.alwaysShowAISparkles: true`); it does not reflect the user's real actions, order, or overrides. It passes its own `PopupHoverState()` and `isStatic: true` to `PopupView` so hovering it never leaks into the real popup's shared hover state. Don't reconnect the preview to the live `ActionCoordinator`.
- **Hover state is one shared singleton plus an opt-in static mode.** The real popup observes `PopupHoverState.shared` (fed by the global mouse monitor). Any other surface that renders `PopupView` must pass its own `hoverState:` instance and `isStatic: true` when it should be inert — `updateHoveredTarget`/`useLocalHoverFallback` early-return for static previews.
- **Shortcut with no selection falls back to the clipboard.** `HotkeyManager` reads `NSPasteboard.general` when the frontmost app yields no selection, then flags `SelectionContext.isClipboardFallback`; `PopupWindowController.show` restricts the popup to Paste + the AI button only. Don't change that filtering without touching the flag.
...
8. **Always verify:** Run `xcodebuild` build and unit test verification commands before declaring completion of any task. Note: the full `./scripts/test.sh` suite can hang in automated sessions — prefer a `timeout -k 5 60 xcodebuild -project OpenClip.xcodeproj -scheme OpenClip -destination 'platform=macOS' build` as a quick compile gate, then run the suite once at the end.
9. **Popup is an LSUIElement NSPanel:** It must never become the key window — no `canBecomeKey`/`canBecomeMain` overrides, no `panel.makeKey()`. Making the panel key steals keyboard focus from the active app and swallows keystrokes. Instead, keyboard dismissal (Escape, or any key closing the popup) runs through the global event monitor, which requires Accessibility permission but is observation-only: the keystroke still lands in the source app's document.
