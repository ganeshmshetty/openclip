...
final class AIActionTests: XCTestCase {
    ...
    func testInsertionGapFollowsRowMidpoints() {
        XCTAssertEqual(AIActionsPage.insertionGap(atY: 0, rowFrames: rowFrames), 0, "above the first row")
        XCTAssertEqual(AIActionsPage.insertionGap(atY: 14, rowFrames: rowFrames), 0, "top half of row 0")
        XCTAssertEqual(AIActionsPage.insertionGap(atY: 16, rowFrames: rowFrames), 1, "bottom half of row 0")
        XCTAssertEqual(AIActionsPage.insertionGap(atY: 46, rowFrames: rowFrames), 2, "bottom half of row 1")
        XCTAssertEqual(AIActionsPage.insertionGap(atY: 400, rowFrames: rowFrames), 4, "below the last row")
        XCTAssertEqual(AIActionsPage.insertionGap(atY: 10, rowFrames: []), 0, "empty list has one gap")
    }

    /// The bar is drawn in the gap — on a row edge, never across a row.
    func testInsertionBarSitsOnTheRowEdges() {
        XCTAssertEqual(AIActionsPage.insertionY(forGap: 0, rowFrames: rowFrames), 0)
        XCTAssertEqual(AIActionsPage.insertionY(forGap: 2, rowFrames: rowFrames), 60)
        XCTAssertEqual(AIActionsPage.insertionY(forGap: 4, rowFrames: rowFrames), 120, "trailing gap sits on the last row's bottom edge")
        XCTAssertNil(AIActionsPage.insertionY(forGap: 0, rowFrames: []))
    }
