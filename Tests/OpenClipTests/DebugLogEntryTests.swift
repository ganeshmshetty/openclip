...
final class DebugLogEntryTests: XCTestCase {
    func testLevelMapsFromOSLogEntryLevel() {
        XCTAssertEqual(DebugLogLevel(OSLogEntryLog.Level(rawValue: 0) ?? .undefined), .debug)
        ...
        XCTAssertEqual(DebugLogLevel(OSLogEntryLog.Level(rawValue: 99) ?? .undefined), .debug)
    }
