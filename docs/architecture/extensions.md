...
| L2+ — network + effects | `js` (`"async": true`) | L2 plus network `fetch`, notifications, `openclip.*` effects (keyPress, runShortcut, openURL, showContent/Status) | call an API and paste the result |
