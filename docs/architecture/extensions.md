...
| L2+ — network + effects | `js` (`"async": true`), `canvas` | L2 plus network `fetch`, notifications, `openclip.*` effects (keyPress, runShortcut, openURL, showStatus). Canvas rendering (`type: "canvas"`) is part of the JS runtime — never a shell effect; `"showContent"` is not accepted by the shell JSON protocol | call an API and paste the result; render an interactive canvas |
