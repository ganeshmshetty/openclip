...
- **Quick Assist** (opt-in, off by default): with Live assist on, OpenClip reads the field you are typing in through Accessibility and answers the Decision tools you marked **Show in Quick Assist** in a small floating window you can drag anywhere (it opens bottom-right and remembers where you leave it). Every tool's question goes in one request, so a System One model answers them all in a single forward pass. Yes/no tools show a tick, cross or question mark; choice tools show the chosen label. Password fields are never read and nothing is stored. The menu bar's **Live Assist** submenu turns it on for 30 minutes, an hour, or until tomorrow, like Pause, and **Always On** mirrors the Settings toggle.
